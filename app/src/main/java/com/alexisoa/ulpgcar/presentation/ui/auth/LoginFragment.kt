...
class LoginFragment : Fragment(R.layout.fragment_login) {
                ...
                viewModel.loginWithMailAndPassword(emailText.text.toString(), passwordText.text.toString()).observe(viewLifecycleOwner, Observer {result: Resource<User> ->
                    when(result){
                        ...
                        is Resource.Success->{
                            ...
                            val actionUser = LoginFragmentDirections.actionLoginFragmentToNavigationSearch(result.data)
                            saveSharedPreferencesUser(result)
    ...
    private fun viewModelAuthWithGoogle(idToken: String) {
        ...
        with(binding){
            viewModel.loginWithGoogleAuth(credential).observe(viewLifecycleOwner, Observer {result: Resource<User> ->
                when(result){
                    is Resource.Loading->{
                        progressBar.visibility = View.VISIBLE
                    }
    ...
    private fun saveSharedPreferencesUser(result: Resource.Success<User>) {
        prefs.saveEmail(result.data.email)
        prefs.saveName(result.data.fullName)
        prefs.saveUID(result.data.uid)
        prefs.saveUrlImage(result.data.imageUrl)
        prefs.saveProvider(result.data.provider)
        prefs.saveDateCreation(result.data.dateRegister.toString())
        prefs.saveTokenApp(result.data.appToken)
        prefs.saveDescription(result.data.description)
    }
}